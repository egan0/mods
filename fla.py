...
def plot_orb_keypoints(pil_image):
    ...
    size = max(2,int(image.shape[0]*downscale*0.010))
    data = list(tuple(i) for i in keypoints)
    y, x = zip(*data)
    l = len(x)
    result = sum(x) / l * downscale, sum(y) / l * downscale
    draw.ellipse([(result[0]-size, result[1]-size), (result[0]+size, result[1]+size)], fill = (255,0,0))
...
def add_histo(img):
    ...
    y = 10#hist_image.height - text_height - 10
...
class CameraInit(Resource):
	...
	#main={"size": (3840, 2160)},
        ...
        try:
            with open("/home/pi/OpenScan/settings/cam_resX", "r") as f:
                resX = int(f.readline()[:20])

            with open("/home/pi/OpenScan/settings/cam_resY", "r") as f:
                resY = int(f.readline()[:20])
        except:
            pass
...
class AutoFocus1(Resource):
    def get(self):
        ...
        #picam2.set_controls({"AfMode": 2, "AfTrigger": 0, "AfRange": picam2.controls.AfRangeEnum.Macro})
